...
func selected_index() -> int:
	...
	return item.get_index() if item != null else -1
...
func sync_display(keep_selection: bool = false) -> void:
	...
	if keep_selection and selected >= 0 and selected < len(items):
		_list.set_selected(root.get_child(selected), 0)
	else:
		_sync_rows(-1)
...
func select_item(index: int) -> void:
	...
	if root != null and index >= 0 and index < root.get_child_count():
		_list.set_selected(root.get_child(index), 0)
