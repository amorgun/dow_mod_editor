...
func _drop_data(at_position: Vector2, data: Variant) -> void:
	...
	if moved_item != target_item:
		...
		undo_redo.add_do_method(func ():
			_move_item(moved_item, new_parent, new_index, new_config_index)
			widget.emit_signal("update")
		)
		undo_redo.add_undo_method(func ():
			_move_item(moved_item, old_parent, old_index, old_config_index)
			widget.emit_signal("update")
		)
...
func _move_item(item: TreeItem, new_parent: TreeItem, new_index: int, new_config_index: int) -> void:
	var item_widget := _get_widget(item)
