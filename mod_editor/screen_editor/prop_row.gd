...
func setup(own_value: Variant, has_own: bool, style_value: Variant, has_style: bool, allow_actions: bool = true) -> void:
	...
	if has_own:
		...
		if allow_actions:
			_add_action_button("Reset", func (): reset_requested.emit(key))
	elif has_style:
		_editor = _make_editor(false)
		_set_editor_value(style_value)
		_last_value = style_value
		if allow_actions:
			_add_action_button("Override", func (): override_requested.emit(key))
	else:
		...
		add_child(_editor)
		if allow_actions:
			_add_action_button("Add", func (): add_requested.emit(key))
	_suppress = false
...
func _emit_changed(value: Variant) -> void:
	if _suppress or value == _last_value:
		return
	_last_value = value
	...
	match descriptor["kind"]:
		...
		ScreenPropDefs.Kind.VEC2:
			if value is Array and len(value) == 2:
				_editor.get_child(0).text = str(value[0])
				_editor.get_child(1).text = str(value[1])
		ScreenPropDefs.Kind.ENUM:
